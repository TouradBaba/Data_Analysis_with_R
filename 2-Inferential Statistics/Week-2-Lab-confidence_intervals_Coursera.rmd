...
ggplot(samp, aes(x = area)) +
  ...
  labs(title = "Distribution of Home Sizes in Sample", x = "Area", y = "Count")
```
...
# The confidence interval bounds from previous calculations
ci_lower <- 1390.291
ci_upper <- 1625.809

# Check if the true mean is within the confidence interval
contains_mu <- ci_lower < true_mean & ci_upper > true_mean
...
sd_area <- sd(samp$area)
ci_99 <- ci %>%
  mutate(lower_99 = lower - (z_star_99 - z_star_95) * (sd_area / sqrt(n)),
         upper_99 = upper + (z_star_99 - z_star_95) * (sd_area / sqrt(n)))

# Create data frame for plotting
ci_data_99 <- data.frame(ci_id = c(1:50, 1:50),
                         ci_bounds = c(ci_99$lower_99, ci_99$upper_99),
                         capture_mu = c(ifelse(ci_99$lower_99 < params$mu & ci_99$upper_99 > params$mu, "yes", "no"),
                                         ifelse(ci_99$lower_99 < params$mu & ci_99$upper_99 > params$mu, "yes", "no")))

# Plot the 99% confidence intervals
ggplot(data = ci_data_99, aes(x = ci_bounds, y = ci_id, 
                              group = ci_id, color = capture_mu)) +
  geom_point(size = 2) +  # add points at the ends
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") +  # draw vertical line for true mean
  labs(title = "99% Confidence Intervals for House Sizes", x = "Area", y = "Sample ID") +
  theme_minimal()
```
