...
# Create side-by-side boxplots of habit and weight
ggplot(nc, aes(x = habit, y = weight)) +
  geom_boxplot() +
  labs(title = "Boxplots of Baby Weights by Mother's Smoking Habit",
       x = "Mother's Smoking Habit",
       y = "Weight of Baby (in pounds)") +
  theme_minimal()
```
...
# Calculate a 90% confidence interval for the average length of pregnancies
inference(y = weeks, data = nc, statistic = "mean", type = "ci", 
          conf_level = 0.90, method = "theoretical")
```
Using a CI of 90% we have (38.1819 , 38.4874) compared to (38.0952 , 38.5742) from the 99 CI.
...
Method:
Identify Age Groups: Use the mature variable to segment the data into "younger mom" and "mature mom".
Find Age Extremes: Compute the maximum age for "younger mom" and the minimum age for "mature mom" using the mage variable.
```{r cutoff-mature}
# type your code for Question 7 here, and Knit
# Determine the maximum age of younger mothers and the minimum age of mature mothers
max_age_younger <- max(nc$mage[nc$mature == "younger mom"], na.rm = TRUE)
min_age_mature <- min(nc$mage[nc$mature == "mature mom"], na.rm = TRUE)
...
```
Explanation:
Maximum Age of Younger Mothers: The max() function is used to find the highest age of mothers classified as "younger mom."
Minimum Age of Mature Mothers: The min() function is used to find the lowest age of mothers classified as "mature mom."
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
# Conduct hypothesis test to evaluate if there is a difference in average birth weight
inference(y = weight, x = whitemom, data = nc, statistic = "mean", 
          type = "ht", null = 0, alternative = "twosided", 
          method = "theoretical")
```
...
**Research Question:** Is there a significant difference in the average birth weight of babies born to white mothers compared to those born to non-white mothers?

**Test Results:**
- **Sample Sizes:** 
  - Non-white mothers: \( n_{\text{not white}} = 284 \)
  - White mothers: \( n_{\text{white}} = 714 \)
- **Sample Means:**
  - Non-white mothers: \( \bar{y}_{\text{not white}} = 6.7195 \)
  - White mothers: \( \bar{y}_{\text{white}} = 7.2505 \)
- **Sample Standard Deviations:**
  - Non-white mothers: \( s_{\text{not white}} = 1.6207 \)
  - White mothers: \( s_{\text{white}} = 1.4333 \)

**Hypotheses:**
- **Null Hypothesis (H0):** The average birth weight of babies born to non-white mothers is equal to that of babies born to white mothers (\( \mu_{\text{not white}} = \mu_{\text{white}} \)).
- **Alternative Hypothesis (HA):** The average birth weight of babies born to non-white mothers is different from that of babies born to white mothers (\( \mu_{\text{not white}} \neq \mu_{\text{white}} \)).

**Test Statistic and p-value:**
- **t-statistic:** -4.8214
- **Degrees of Freedom (df):** 283
- **p-value:** < 0.0001

**Conclusion:**
Since the p-value is much less than the significance level of 0.05, we reject the null hypothesis. This suggests that there is a significant difference in the average birth weight of babies born to white mothers compared to those born to non-white mothers. Specifically, babies born to white mothers tend to have a higher average birth weight.
