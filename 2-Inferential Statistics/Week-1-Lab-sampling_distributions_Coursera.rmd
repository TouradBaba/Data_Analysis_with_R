...
samp1_stats <- samp1 %>%
  summarise(
    sample_mean = mean(area),
    sample_median = median(area),
    sample_sd = sd(area),
    sample_iqr = IQR(area),
    sample_min = min(area),
    sample_max = max(area),
    sample_q1 = quantile(area, 0.25),
...
num_elements <- nrow(sample_means50)

# Summary statistics for the sampling distribution
sampling_dist_stats <- sample_means50 %>%
  summarise(
    mean_x_bar = mean(x_bar),
    sd_x_bar = sd(x_bar)
  )

# Print the number of elements and summary statistics
print(num_elements)
...
# type your code for the Exercise here, and Run Document
# Create a sampling distribution of 25 sample means from samples of size 10
...
# Count the number of rows in sample_means_small
num_elements <- nrow(sample_means_small)
...
price_sample <- sample(ames$price, 50) 

# Calculate the best point estimate of the population mean
point_estimate <- mean(price_sample)
...
sample_means50 <- replicate(num_samples, {
  sample_mean <- mean(sample(ames$price, sample_size))
  return(sample_mean)
})

# Convert to a data frame for plotting
sample_means_df <- data.frame(sample_mean = sample_means50)
...
ggplot(sample_means_df, aes(x = sample_mean)) +
  ...
  ggtitle("Sampling Distribution of Sample Means (n = 50)") +
  xlab("Sample Mean of Price") +
  ylab("Frequency") +
  theme_minimal()

# Print summary statistics of the sampling distribution
...
# Simulate the sampling distribution for sample size 150
sample_means150 <- replicate(num_samples, {
  sample_mean <- mean(sample(ames$price, new_sample_size))
  return(sample_mean)
})

# Convert to a data frame for plotting
sample_means150_df <- data.frame(sample_mean = sample_means150)

# Plot the distribution of sample means with sample size 150
library(ggplot2)
ggplot(sample_means150_df, aes(x = sample_mean)) +
  geom_histogram(binwidth = 10000, fill = "lightgreen", color = "black") +
  ggtitle("Sampling Distribution of Sample Means (n = 150)") +
  xlab("Sample Mean of Price") +
  ylab("Frequency") +
  theme_minimal()
...
# Optionally, compare with previous sample size 50
# (Ensure you have the data from the previous part)
summary(sample_means50)
```
...
# Take a random sample of size 15 from the price column
price_sample_small <- sample_n(ames, 15) %>% 
  pull(price) # Extract the price column from the sample

# Calculate the mean of the sampled prices
point_estimate_small <- mean(price_sample_small)

# Print the point estimate
point_estimate_small
```
...
sample_means15 <- numeric(2000)

# Set a seed for reproducibility
set.seed(123)

# Take 2000 samples, each of size 15
for (i in 1:2000) {
  sample_means15[i] <- mean(sample_n(ames, 15) %>% pull(price))
}

# Plot the sampling distribution
ggplot(data.frame(sample_means15), aes(x = sample_means15)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  labs(title = "Sampling Distribution of the Mean Home Price (Sample Size = 15)",
       x = "Sample Mean Price",
       y = "Frequency")

# Calculate the population mean
population_mean <- mean(ames$price)
...
# type your code for this Exercise here, and Run Document
# Initialize a vector to store sample means
sample_means150 <- numeric(2000)

# Set a seed for reproducibility
set.seed(123)

# Take 2000 samples, each of size 150
for (i in 1:2000) {
  sample_means150[i] <- mean(sample_n(ames, 150) %>% pull(price))
}

# Plot the sampling distribution
ggplot(data.frame(sample_means150), aes(x = sample_means150)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  labs(title = "Sampling Distribution of the Mean Home Price (Sample Size = 150)",
       x = "Sample Mean Price",
       y = "Frequency")
...
# Compute the standard deviation of the sampling distributions
sd_sample_means50 <- sd(sample_means50)
sd_sample_means150 <- sd(sample_means150)

# Print standard deviations
sd_sample_means50
sd_sample_means150

# Compare the means of the sampling distributions
mean_sample_means50 <- mean(sample_means50)
mean_sample_means150 <- mean(sample_means150)

mean_sample_means50
mean_sample_means150

# Plot the distributions to check symmetry
p1 <- ggplot(data.frame(sample_means50), aes(x = sample_means50)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  labs(title = "Sampling Distribution of the Mean Home Price (Sample Size = 50)",
       x = "Sample Mean Price",
       y = "Frequency")

p2 <- ggplot(data.frame(sample_means150), aes(x = sample_means150)) +
