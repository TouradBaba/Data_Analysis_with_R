...
ggplot(data = mlb11, aes(x = at_bats, y = runs)) +
  ...
  labs(title = "Relationship between Runs and At Bats",
       x = "At Bats",
       y = "Runs")
```
...
If the relationship between at_bats and runs appears to be linear based on the scatterplot, it suggests that a linear model could be a reasonable choice for prediction. However, comfort with using a linear model also depends on checking other assumptions such as:

Linearity: The relationship should be linear, which you have observed.
Independence: Observations should be independent.
Homoscedasticity: The residuals should show constant variability.
Normality: Residuals should be normally distributed.
If these assumptions hold true, then yes, it would be comfortable to use a linear model for prediction.
...
After running the function multiple times, the smallest sum of squares I obtained is 125,573.6.
...
# Summary to see coefficients
summary(m2)

at_bats <- 5579

# Calculate predicted runs
predicted_runs <- -2789.2429 + 0.6305 * at_bats
...
# Scatterplot of runs vs. hits
ggplot(mlb11, aes(x = hits, y = runs)) +
  geom_point() +
  geom_smooth(method = "lm", col = "blue") +
  labs(title = "Scatterplot of Runs vs. Hits", x = "Hits", y = "Runs")
...
```{r pick-model}
# type your code for the Exercise here, and Knit
# Assuming you have the model from the previous at_bats exercise
summary_at_bats <- summary(lm(runs ~ at_bats, data = mlb11))

# Extract R² values
r_squared_hits <- summary(model_hits)$r.squared
r_squared_at_bats <- summary_at_bats$r.squared

r_squared_hits
r_squared_at_bats
```
The new variable (hits) has a higher R², indicating that it offers better predictive power than the other variable.
...
# List of other traditional variables
variables <- c("at_bats", "hits", "wins", "bat_avg")
r_squared_values <- data.frame(variable = variables, r_squared = NA)

# Loop through each variable, fit a model, and store R-squared values
for (var in variables) {
  model <- lm(as.formula(paste("runs ~", var)), data = mlb11)
  r_squared_values[r_squared_values$variable == var, "r_squared"] <- summary(model)$r.squared
  
  # Create scatterplot and add regression line
  plot(mlb11[[var]], mlb11$runs,
       main = paste("Runs vs", var),
       xlab = var,
       ylab = "Runs")
...
From the graphs and R² values for the three variables, it is clear that batting average (bat_avg) performs better than the other two.
...
# Newer variables
new_variables <- c("new_obs", "new_slug", "new_onbase")
r_squared_new <- data.frame(variable = new_variables, r_squared = NA)

for (var in new_variables) {
  model <- lm(as.formula(paste("runs ~", var)), data = mlb11)
  r_squared_new[r_squared_new$variable == var, "r_squared"] <- summary(model)$r.squared
}
...
```{r diag}
# type your code for the Exercise here, and Knit
# Fit the model with new_obs
best_model <- lm(runs ~ new_obs, data = mlb11)

# Check diagnostics
par(mfrow=c(2,2)) # to plot multiple graphs
plot(best_model)
```
